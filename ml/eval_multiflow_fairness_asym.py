#!/usr/bin/env python3
"""
ml/eval_multiflow_fairness_asym.py

Asymmetric two-flow fairness evaluation for congestion-control baselines.
...
Asymmetric cases:
1. equal_start_equal_rate
2. late_joiner
3. unequal_initial_rates
4. rtt_mismatch_proxy

Outputs:
- results/multiflow_fairness_asym_summary.csv
- results/multiflow_fairness_asym_summary.json
"""
...
def controller_next_rate(name, state, min_rate, max_rate, alpha, bundle=None, ppo_model=None):
    ...
    if name == "EWMA_SR":
        ...
        return float(np.clip(ewma_thr, min_rate, max_rate))
...
def make_case_config(case_name, init_sr_base):
    if case_name == "equal_start_equal_rate":
        return {
            "join_step_1": 0,
            "join_step_2": 0,
            "init_sr_1": init_sr_base,
            "init_sr_2": init_sr_base,
            "rtt_weight_1": 1.0,
            "rtt_weight_2": 1.0,
        }
    if case_name == "late_joiner":
        return {
            "join_step_1": 0,
            "join_step_2": 20,
            "init_sr_1": init_sr_base,
    ...
    if case_name == "unequal_initial_rates":
        return {
            "join_step_1": 0,
            "join_step_2": 0,
            "init_sr_1": max(2.0, init_sr_base * 0.6),
            "init_sr_2": min(12.0, init_sr_base * 1.4),
            "rtt_weight_1": 1.0,
            "rtt_weight_2": 1.0,
        }
    if case_name == "rtt_mismatch_proxy":
        return {
            "join_step_1": 0,
            "join_step_2": 0,
            "init_sr_1": init_sr_base,
...
def shared_bottleneck_step_asym(bottleneck, send_rates, q_bytes_total, rng, rtt_weights):
    send_rates = np.array(send_rates, dtype=float)
    weights = np.array(rtt_weights, dtype=float)
    weighted_offer = send_rates * weights
    offered = np.sum(weighted_offer)

    if offered <= 1e-9:
        throughputs = np.zeros_like(send_rates)
    elif offered <= bottleneck:
        throughputs = weighted_offer.copy()
    else:
        throughputs = bottleneck * (weighted_offer / offered)

    noise = rng.normal(0.0, 0.03 * max(1e-6, bottleneck), size=len(send_rates))
    throughputs = np.maximum(0.0, throughputs + noise)

    excess = max(0.0, np.sum(send_rates) - bottleneck)
    q_bytes_total = max(0.0, q_bytes_total + excess * 8000.0)
    q_packets_total = q_bytes_total / 1200.0 if q_bytes_total > 0 else 0.0
...
def run_multiflow_case(df, controller_name, case_name, min_rate, max_rate, alpha, rng, bundle=None, ppo_model=None):
    bottleneck = float(df["bottleneck_bw_mbps"].iloc[0])
    base_sr = float(np.clip(df["send_rate_mbps"].iloc[0], min_rate, max_rate))
    cfg = make_case_config(case_name, base_sr)

    init_thr = min(base_sr, bottleneck / 2.0)

    flow1 = {
        "send_rate": cfg["init_sr_1"],
        "last_thr": init_thr,
        "queue_bytes": 0.0,
        "ewma_thr": init_thr,
        "recent_thr": [init_thr],
        "phase_idx": 0,
        "history": deque(build_initial_history(init_thr, cfg["init_sr_1"]), maxlen=WINDOW),
        "bottleneck": bottleneck,
    }
    flow2 = {
        "send_rate": cfg["init_sr_2"],
        "last_thr": init_thr,
        "queue_bytes": 0.0,
        "ewma_thr": init_thr,
        "recent_thr": [init_thr],
        "phase_idx": 0,
        "history": deque(build_initial_history(init_thr, cfg["init_sr_2"]), maxlen=WINDOW),
        "bottleneck": bottleneck,
    }
    ...
    for i in range(len(df)):
        ...
        active1 = i >= cfg["join_step_1"]
        active2 = i >= cfg["join_step_2"]
        ...
        thrs, q_total, q_packets_total = shared_bottleneck_step_asym(
            bottleneck=bottleneck,
            send_rates=[sr1, sr2],
            q_bytes_total=q_total,
            rng=rng,
            rtt_weights=[cfg["rtt_weight_1"], cfg["rtt_weight_2"]],
        )
        ...
        fairness_now = jain_fairness([thr1, thr2]) if active1 and active2 else 0.0
        ...
        rows.append({
            ...
            "fairness": fairness_now,
            "controller": controller_name,
            "flow1_active": int(active1),
            "flow2_active": int(active2),
        })

    return pd.DataFrame(rows)


def summarize_case(scenario, controller, case_name, simdf):
    both_active = simdf[(simdf["flow1_active"] == 1) & (simdf["flow2_active"] == 1)].copy()
    if both_active.empty:
        both_active = simdf.copy()

    return {
        "scenario": scenario,
        "controller": controller,
        "case": case_name,
        "n": int(len(both_active)),
        "avg_total_throughput": float(both_active["total_thr"].mean()),
        "avg_flow1_throughput": float(both_active["flow1_thr"].mean()),
        "avg_flow2_throughput": float(both_active["flow2_thr"].mean()),
        "avg_fairness": float(both_active["fairness"].mean()),
        "avg_queue_bytes": float(both_active["queue_bytes"].mean()),
        "p95_queue_bytes": float(np.percentile(both_active["queue_bytes"], 95)),
        "avg_send_rate": float((both_active["flow1_send"].mean() + both_active["flow2_send"].mean()) / 2.0),
    }
...
def main():
    ...
    cases = [
        "equal_start_equal_rate",
        "late_joiner",
        "unequal_initial_rates",
        "rtt_mismatch_proxy",
    ]
    ...
    for idx, path in enumerate(files):
        ...
        for j, ctrl in enumerate(controllers):
            for k, case_name in enumerate(cases):
                simdf = run_multiflow_case(
                    df=df,
                    ...
                    alpha=args.alpha,
                    rng=np.random.default_rng(args.seed + idx * 100 + j * 10 + k),
                    bundle=bundle,
                    ppo_model=ppo_model,
                )
                summaries.append(summarize_case(scenario, ctrl, case_name, simdf))

    out_df = pd.DataFrame(summaries)
    os.makedirs("results", exist_ok=True)

    csv_path = "results/multiflow_fairness_asym_summary.csv"
    json_path = "results/multiflow_fairness_asym_summary.json"
    ...
    overall = (
        out_df.groupby(["case", "controller"], as_index=False)[
            ["avg_total_throughput", "avg_fairness", "avg_queue_bytes", "p95_queue_bytes", "avg_send_rate"]
        ]
        .mean()
        .sort_values(["case", "avg_total_throughput"], ascending=[True, False])
    )

    print("\n=== Asymmetric Multi-flow Overall ===")
